...
- 手动试跑：在 GitHub Actions 页面运行 `Release Plugin` workflow，各平台产物会作为 workflow artifact 上传。
- 正式发布：推送 `v*` tag，例如 `v0.2.0`，workflow 会构建 `paddle-ocr-v*-<target>.zip` 并上传到该 tag 对应的 GitHub Release。
